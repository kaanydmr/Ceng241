...
int deleteStudent(StudentList* list, int studentNumber) {
    ...
    for (int a = 0; a < (list->studentCount); a++) {
        ...
        if (list->students[a].studentNumber == studentNumber) {
            ...
            for (int b = a; b < (list->studentCount - 1); b++) {
...
void sort(StudentList* list) {		
    ...
    for (int a = 0; a < list->studentCount - 1; a++) {
    	
        for (int b = 0; b < list->studentCount - a - 1; b++) {
...
int main() {
    ...
    if (deleted == 1) {
        ...
        printf("Enter Student ID to be deleted: 2022");
        printf("\nSuccesfuly deleted student\n\n");
    } 
	else if(deleted == -1) {
		
        printf("\nStudent with student number 2022 not found.\n");
    }

    printStudentList(slist);

    //Free memory for students and their courses
    for (int a = 0 ; a < slist.studentCount; a++) {
